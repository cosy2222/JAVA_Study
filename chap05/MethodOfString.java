...
public class MethodOfString {
	...
	public static void main(String[] args) {
		// 1 . 문자열 길이 ( length() ) : int xkdlqdmfh qusghks
		...
		// indexOf() : 0번쨰 방에서 부터 마지막 방까지의 글자의 방번호(index)를 출력
		// lastIndexOf() : 마지막 방부터 0번쨰 방으로 글자의 방번호를 출력
		...
		System.out.println(str1.lastIndexOf("Java"));  // 6번방 부터 Java 문자열이 시작
		System.out.println(str2.indexOf("하세요"));
	    System.out.println(str2.lastIndexOf("하세요"));
	    System.out.println(str2.lastIndexOf("bye"));    //  <<중요>> -1 : 값이 존재하지 않는 경우
	    System.out.println();
	    
	    
	    //3. 문자열 변환 및 연결 ( String.valueOf(),cacat())
	    // String.valueOf (기본자료형)   : 기본자료형 ==> String 으로 변환
	    String str3 = String.valueOf(2.3);    // 실수 2.3.을 String으로 변환
	    String str4 = String.valueOf(false);   // boolean 타입 false 를 String 으로 변환
	    ...
	    String str5 = str3.concat(str4);     // str5 =  str3 + str4
	    System.out.println(str5);            //  2.3false
	    System.out.println();
	    
	    
	    // concat() 메소드에서 String.valueOf 사용  ===>   +는 자동으로 타입변환
	    String str6 = "안녕" + 3;     // 안녕3
	    String str7 = "안녕".concat(String.valueOf(3)); // 안녕3
	    			//concat을 사용할떄 String 타입으로 변환 필요
	    ...
	    // 4. 문자열 == > byte[] 타입으로 변환 , 입출력 값을 처리할떄 사요ㅕㅇ
	    	// (getBytes(),
