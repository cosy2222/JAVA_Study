...
// 메소드 오버라이딩 ( MethodOverriding ) :
	...
	// 2. 부머 클래스에서 선언된 메소드를 자식 클래스에서 새롭게 정의해서 사용하는것 .
...
class B extends A {
	//@Override   // 메소드 오버라이딩시 , 반드시 부모의 메소드를 재정의 하도록 설정
			      // @Override 어노테이션을 사용하지 않을 경우 : 오타가 나거나 형식에 맞지 않을경우 
				  //                                     별갸의 메소드로 정의됨.
	void print() {
...
public class MethodOverriding {
	...
	public static void main(String[] args) {
		...
		//3. A타입 업캐스팅     <<=== B를 업캐스팅 A 타입으로 두면 
		A ab = new B();
		ab.print();         // B클래스의 print() 출력
			// ab.print() 는 A클래스의 print를 호출 , 동적 방인딩에 의해서 B클래스의 print()를 출력
