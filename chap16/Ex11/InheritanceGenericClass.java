...
// 제너릭 클래스의 상속 : 자식클래스는 부모클래스의 제너릭 변수 (K,V,T,E) 와 같거나 더 많아야 한다 
...
class Child1 <T> extends Parent<T>{  // 제너릭 클래스를 상속받는 자식클래스는 부모의 타입의 인자갯수와 같거나 커야한다 
...
//		Parent<String> p = new Parent();			// 뒤의 생성자호출할떄는 생략 가능
